...
//布局换色设置 #575757
        ...
        $("div.bgSelector").toggle(300, function() {
            if ($(this).html() == '') {
                $(this).sColor({
                    ...
                    defaultItem: ($.cookie('bgColorSelectorPosition') != null) ? $.cookie('bgColorSelectorPosition') : 0  // 可选，第几个颜色的索引作为初始颜色，默认第1个颜色
                });
            }
        });//切换显示
    });    
    ...
    if ($.cookie('commonPendingMatters') == null) {
        $.ajax({
                url : '?act=common&op=ajax_pending_matters',
                async : false,
...
function openItem(param) {
    ...
    other_str = data_str[4]?data_str[4]:'';
	...
	$('#workspace').attr('src',  ADMIN_SITE_URL+'&c=' + data_str[0] + '&a=' + data_str[1] + '&menu_id=' + data_str[3]+'&'+other_str);
	$.cookie('workspaceParam', data_str[0] + '|' + data_str[1] + '|' + data_str[2] + '|' + data_str[3]+'|'+other_str, { expires: 1 ,path:"/"});
}