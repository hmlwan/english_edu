		...
		<table class="flex-table">
			<thead>
				<tr>
					...
					<th width="300" align="center">学科名称</th>
					<th width="300" align="center">年级</th>
			...
			<tbody>
				<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$r): $mod = ($i % 2 );++$i;?><tr data-id="<?php echo ($r["id"]); ?>">
					...
					<td class="handle">
					<a class="btn red" href="<?php echo U('Discipline/del',array('id'=>$r['id']));?>" onclick="if(confirm('删除后将不能恢复，确认删除这 1 项吗?')){return true;} else {return false;}"><i class="fa fa-trash-o"></i>删除</a>
						<a class="btn blue" href="<?php echo U('Discipline/edit',array('id'=>$r['id']));?>"><i class="fa fa-pencil-square-o"></i>编辑</a>
					</td>
					...
					<td class="name"><span title="可编辑" column_id="<?php echo ($r["id"]); ?>" fieldname="name" nc_type="inline_edit" class="editable "><?php echo ($r["name"]); ?></span></td>
					</td>
	...
	<script>
		$(function(){
			...
			function fg_operation(name,bDiv){
				if(name == 'add') {
					window.location.href = '<?php echo U("Discipline/add");?>';
				}else if(name == 'del'){
		...
		function fg_del(ids){
			...
			var id = ids.join(',');
			if(confirm('确认这些商品吗？')){
				$.getJSON('<?php echo U("Discipline/del");?>',{'id':id},function(data){
					if(data.status) {
						location.reload();
					}else {
						showError(data.info);
