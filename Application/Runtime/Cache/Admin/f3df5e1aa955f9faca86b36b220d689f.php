	...
	<div class="page" style="padding:30px 1% 0 1%;">
		...
		<div class="mDiv">
			<div class="sDiv">
				...
				<div class="sDiv2">
				<form action="<?php echo fn_getUrl();?>" method='get'>
		...
		<table class="flex-table">
			...
			<thead>
				<tr>
					...
					<th width="120" align="center">学生姓名</th>
					<th width="120" align="center">所在年级</th>
					<th width="120" align="center">所在班级</th>
			...
			<tbody>
				<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr data-id="<?php echo ($vo["admin_id"]); ?>">
					...
					<td class="handle">
						...
						<a class="btn blue" href="<?php echo U('userEdit',array('id'=>$vo['admin_id']));?>"><i class="fa fa-pencil-square-o"></i>编辑</a>

						<!--<a class="btn blue" href="<?php echo U(CONTROLLER_NAME.'/userDetail',array('id'=>$vo['id']));?>"><i class="fa fa-file-o"></i>详情</a>-->
					<!--<a class="btn green" href="<?php echo U(CONTROLLER_NAME.'/logData',array('id'=>$vo['id']));?>"><i class="fa fa-pencil-square-o"></i>录入</a>-->
					<!--<a class="btn orange" href="<?php echo U(CONTROLLER_NAME.'/logData',array('id'=>$vo['id'],'type'=>'print'));?>"><i class="fa fa-print"></i>打印</a>-->
					</td>
					<td class="name"><?php echo ($vo["admin_id"]); ?></td>
					<td class="name"><?php echo ($vo["admin_name"]); ?></td>
					<td class="name"><?php echo ($vo["grate_name"]); ?></td>
					<td class="name"><?php echo ($vo["class_name"]); ?></td>
		...
		<div class="page-main quotes"><?php echo ($page); ?>
			<form  method="post" action="<?php echo U('importData');?>" id="myform" style="float: right;padding-top:0">
				...
				<span style="color:#f52d08;"> 导入学生数据：</span><input style="width: 160px" class="uploadFile" name="file_stu" type="file" value="">
...
// 				$("#myform").submit();
		...
		$(function(){
			$('.flex-table').flexigrid({
				...
				usepager:false,
				buttons : [
//					{display: '<i class="fa fa-plus"></i>导入数据', name : 'file_stu', bclass : 'add',title : '导入学生数据',url:"<?php echo U(CONTROLLER_NAME.'/importData');?>", onpress : fg_operation },
